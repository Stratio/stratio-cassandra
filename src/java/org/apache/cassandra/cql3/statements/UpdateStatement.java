...
public class UpdateStatement extends ModificationStatement
{
    ...
    public List<IMutation> getMutations(ClientState clientState, List<ByteBuffer> variables) throws InvalidRequestException
    {
        ...
        if (cfDef.isComposite())
        {
            ...
            for (CFDefinition.Name name : cfDef.columns.values())
            {
                ...
                {
                    firstEmpty = name;
                    if (cfDef.kind == CFDefinition.Kind.SPARSE)
                        throw new InvalidRequestException(String.format("Missing mandatory PRIMARY KEY part %s", name.name));
                }
     ...
     */
    private IMutation mutationForKey(CFDefinition cfDef, ClientState clientState, ByteBuffer key, CompositeType.Builder builder, List<ByteBuffer> variables)
        ...
        switch (cfDef.kind)
        {
            ...
            case SPARSE:
                for (CFDefinition.Name name : cfDef.metadata.values())
                {
                    ...
                    ByteBuffer colName = builder == null
                                       ? name.name.key
                                       : builder.copy().add(name.name.key).build();
            ...
            case DYNAMIC:
                ...
                assert colValues.size() == 1;
                ByteBuffer colName = colValues.get(0).getByteBuffer(name.type, variables);
                ...
                hasCounterColumn = addToMutation(clientState, rm, colName, cfDef.value, value, variables);
                break;
            case DENSE:
                value = processedColumns.get(cfDef.value.name);
                if (value == null)
                    throw new InvalidRequestException(String.format("Missing mandatory column %s", cfDef.value));
