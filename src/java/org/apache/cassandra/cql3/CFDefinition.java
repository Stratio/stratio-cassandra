...
{
    /*
     * We distinguish 4 kinds of column family from a CQL perspective:
     *   - STATIC: non-composite CF with a finite set of UTF8 column defined
     *   - DYNAMIC: non-composite wide row CF (typically time series)
     *   - SPARSE: composite 'sparse' CF. The last component of the composite takes only a
     *             finite set of UTF8 defined value (Emulates 'static' SuperColumns).
     *   - DENSE: composite 'dense' CF. The composite version of DYNAMIC.
     */
    public enum Kind { STATIC, DYNAMIC, SPARSE, DENSE }
    ...
    public final Map<ColumnIdentifier, Name> columns = new LinkedHashMap<ColumnIdentifier, Name>();
    ...
    private final Name columnNameForDynamic; // Cache the unique column alias for the dynamic case to avoid creating an iterator when we need it
    ...
    public CFDefinition(CFMetaData cfm)
    {
        ...
        if (cfm.comparator instanceof CompositeType)
        {
            ...
            this.columnNameForDynamic = null;
            if (!cfm.getColumn_metadata().isEmpty())
            {
                // "sparse" composite
                this.kind = Kind.SPARSE;
                this.value = null;
        ...
        else
        {
            ...
            else
            {
                ...
                this.columnNameForDynamic = name;
    ...
    {
        return kind == Kind.SPARSE || kind == Kind.DENSE;
    }

    public boolean isCompact()
    {
        return kind == Kind.DYNAMIC || kind == Kind.DENSE;
    }

    public Name columnNameForDynamic()
    {
        assert kind == Kind.DYNAMIC;
        return columnNameForDynamic;
    }
