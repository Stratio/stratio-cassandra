 ...
 * 
 * Two functionalities had been added to this subclass. 1) The execute method of the
 * ThreadPoolExecutor will block in case the queue is full and only unblock when the queue is
 * dequeued - that is a task that is currently in the queue is removed and handled by the
 * ThreadPoolExecutor. 2) Client code can await for the event of all tasks beeing run to conclusion.
 * Client code which actively chose to wait for this occurrence should call await on the instance of
 * his ThreadPoolExecutor. This differs from awaitTermination as it does not require any call to
 * shutdown.
 * 
 * Code example:
 * 
 ...
 * thread pool have concluded. their run.
 * 
...
public class NotifyingBlockingThreadPoolExecutor extends ThreadPoolExecutor {
	 ...
	 * all the tasks that have been submitted to the execute() method have run to conclusion. This
	 * notification can occur a numerous amount of times. It is all up to the client code. Whenever
	 * the ThreadPoolExecutor concludes to run all the tasks the Synchronizer object will be
	 * notified and will in turn notify the code which is waiting on it.
	 */
	private Synchronizer synchronizer = new Synchronizer();

	/**
	 * This constructor is used in order to maintain the first functionality specified above. It
	 * does so by using an ArrayBlockingQueue and the BlockThenRunPolicy that is defined in this
	 * class. This constructor allows to give a timeout for the wait on new task insertion and to
	 * react upon such a timeout if occurs.
	 * 
	 * @param poolSize
	 *            is the amount of threads that this pool may have alive at any given time
	 * @param queueSize
	 *            is the size of the queue. This number should be at least as the pool size to make
	 *            sense (otherwise there are unused threads), thus if the number sent is smaller,
	 *            the poolSize is used for the size of the queue. Recommended value is twice the
	 *            poolSize.
	 * @param keepAliveTime
	 ...
	 *            is the maximum time to wait on the queue of tasks before calling the
	 *            BlockingTimeout callback
	 * @param maxBlockingTimeUnit
	 *            is the unit of time to use with the previous parameter
	 * @param blockingTimeCallback
	 *            is the callback method to call when a timeout occurs while blocking on getting a
	 *            new task, the return value of this Callable is Boolean, indicating whether to keep
	 ...
	 * This constructor is used in order to maintain the first functionality specified above. It
	 * does so by using an ArrayBlockingQueue and the BlockThenRunPolicy that is defined in this
	 * class. Using this constructor, waiting time on new task insertion is unlimited.
	 * 
	 * @param poolSize
	 *            is the amount of threads that this pool may have alive at any given time.
	 * @param queueSize
	 ...
	 * in process is decremented. Finally, if the amount of tasks currently running is zero the
	 * synchronizer's signallAll() method is invoked, thus anyone awaiting on this instance of
	 * ThreadPoolExecutor is released.
	 * 
	...
	/**
	 * A blocking wait for this ThreadPool to be in idle state, which means that there are no more
	 * tasks in the Queue or currently executed by one of the threads. BE AWARE that this method may
	 * get out from blocking state when a task is currently sent to the ThreadPool not from this
	 * thread context. Thus it is not safe to call this method in case there are several threads
	 * feeding the TreadPool with tasks (calling execute). The safe way to call this method is from
	 * the thread that is calling execute and when there is only one such thread. Note that this
	 * method differs from awaitTemination, as it can be called without shutting down the
	 * ThreadPoolExecuter.
	 * 
	...
	/**
	 * A blocking wait for this ThreadPool to be in idle state or a certain timeout to elapse. Works
	 * the same as the await() method, except for adding the timeout condition.
	 * 
	 * @see NotifyingBlockingThreadPoolExecutor#await() for more details.
	 * @return false if the timeout elapsed, true if the synch event we are waiting for had
	 *         happened.
	 * @throws InterruptedException
	 *             when the internal condition throws it.
	 */
	 ...
	 * It does so by invoking the BlockingQueue's put method (instead of the offer method that is
	 * used by the standard implementation of the ThreadPoolExecutor - see the opened Java 6 source
	 * code).
	 */
